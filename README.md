...
한국학중앙연구원 *한국향토문화전자대전* **18종 전 분야 공인 데이터(총 5,161건)**와 **2-Layer 백엔드 멀티 에이전트 시스템**을 결합하여, 단순 검색이나 딱딱한 백과사전식 정보가 아닌, **검증된 팩트만 추린 200~300자 핵심 요약**으로 들려줍니다.
...
* **Layer 2: 핵심 요약 에이전트 (`SummaryAgent`)**
...
### 2. 📌 핵심 요약 에이전트 (환각 방지 우선 설계)
| 항목 | 내용 |
|:---|:---|
| **에이전트** | `SummaryAgent` (📌 핵심 요약 에이전트) — 전체 2,692곳 담당 |
| **출력 규격** | 서두·종두 인사 없이 본론부터, 공백 포함 **200~300자** |
| **환각 방지** | 폐사지·비지정 유적지에 임의 관광 추천 수식어 금지, 브리핑에 없는 연도·치수·전설 결말 날조 금지 |
| **설화 우선** | 설화/전설/신화 태그가 있으면 행정 연혁·면적 수치보다 **스토리텔링 줄거리** 우선 서술 |
| **가독성** | 응회환·절리 등 전문 용어를 일상 표현으로 풀어 설명 |
...
### 6. 📒 방명록 — 이웃의 제주 이야기 (Firestore 중앙 저장)
* 명소별로 사용자가 이야기·추억·현장 팁을 남기고 **모든 사용자가 실시간으로 공유**
* 브라우저 `localStorage` 가 아닌 **Firestore 중앙 저장** — 다른 기기·다른 사용자에게 즉시 반영
* 공감(♥)은 `FieldValue.increment` **원자 갱신** — 동시 요청에도 갱신 손실 없음
  *(실측: 병렬 +1 10회 → 정확히 10 / 글 작성 후 20회 분산 조회 → 20/20 일치)*
* 사진 첨부 시 캔버스로 1280px 축소 후 **비공개 GCS 버킷**에 저장, 문서에는 경로만 기록
* 쓰기 요청 IP 당 분당 20회 레이트 리밋, 관리자 삭제는 토큰 검증

---

## 💾 데이터 저장 구조 (Data Layer)

대용량 자료는 컨테이너 이미지가 아니라 **외부 관리형 저장소**에 둔다.

| 저장소 | 대상 | 접근 방식 |
|:---|:---|:---|
| **Cloud Storage** `-data` | 학술 코퍼스 5,161건 · POI 공간 데이터 | 서버 기동 시 1회 적재 → 메모리 상주 |
| **Cloud Storage** `-assets` | POI 인덱스/카드/상세 조각 · 방명록 사진 | 서버가 서비스 계정으로 중계 (`/data/*`, `/media/*`) |
| **Firestore** | 방명록 글 · 공감 수 | `/api/stories` CRUD |
| **Secret Manager** | Gemini · 관리자 · 카카오맵 키 | 런타임 주입 (카카오만 빌드타임) |

**두 GCS 버킷 모두 비공개**이며, 브라우저는 백엔드 프록시를 통해서만 접근한다.
읽기 전용 코퍼스는 GCS, 여러 인스턴스가 동시에 쓰는 방명록은 Firestore로 분리했다.
...
graph TD
    ...
    subgraph "2-Layer Multi-Agent Engine"
        ...
        Summary["📌 SummaryAgent<br/>(핵심 요약 에이전트)"]
    end
    ...
    Orchestrator -->|"5. 요약 생성 지시"| Summary
    Summary -->|"6. 요약 답변 생성"| Orchestrator
...
│   │   │   └── personaAgents/    # Layer 2 summaryAgent.ts (단일)
│   │   │       ├── seolmundaeAgent.ts
│   │   │       ├── haenyeoAgent.ts
