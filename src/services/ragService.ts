...
`;
  ...
  public static buildDeepStoryPrompt(
    ...
    const languageInstruction = `[언어 및 문체 규칙 - 표준어 구술 모드]:
...
[서사 작성 지침 - 서두/종두 사설 최소화 & 핵심 본론 집중]:
- **서두 (도입)**: 장황한 인사말이나 풍경 감상은 완전히 배제하고, **단 1문장**으로 짧게 인사를 전하며 바로 본론 핵심 이야기로 진입하세요.
- **본론 (핵심 이야기 - 80% 이상 집중)**: RAG 데이터베이스의 **관련 구비 설화 원문, 역사적 사건 및 인물, 지질학적 형성 비밀, 1KM 주변 문화유산** 팩트를 몰입감 넘치고 생생하게 구술하세요.
- **종두 (결말)**: 과도한 감상이나 길어지는 교훈 대신, 핵심 여운을 담은 **1~2문장**으로 군더더기 없이 간결하게 마무리하세요.

${languageInstruction}

절대 RAG 지식베이스에 없는 허위 사실을 날조하지 마세요. 소제목이나 라벨('1막', '2막' 등)은 절대로 본문에 출력하지 마세요.
`;
  }
}
