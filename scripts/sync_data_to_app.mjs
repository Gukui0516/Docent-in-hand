...
function isValidPoi(it, title, meta) {
  ...
  // 9. 행정구역 그 자체는 방문 대상이 아니다.
  //
  // 원본 유형 '지명/행정 지명과 마을' 278건 중, 이름이 순수 행정구역명인
  // 212건(우도면, 탑동, 온평리 …)을 제외한다. 기존 필터는 '지명/도로' 만
  // 걸러 이들이 전부 관광지로 통과했다.
  //
  // 유형과 이름 형태를 함께 본다. 이름만 보면 종다리(새)·고사리(식물)·
  // 고태리(인물)처럼 '리'로 끝나는 다른 항목까지 걸린다. 반대로 유형만 보면
  // '제주 성읍마을'·'산천단' 같은 실제 방문지 66건이 사라진다.
