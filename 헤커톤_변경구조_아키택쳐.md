...
graph TD
    ...
    subgraph "⚡ Backend Server (Node.js Express Agent Runtime)"
        ...
        subgraph "Layer 2: 📌 핵심 요약 에이전트 (Summary Agent)"
            SummaryAgent[📌 SummaryAgent<br>• 브리핑 노트를 200~300자 팩트 요약으로 변환<br>• 서두·종두 인사 금지, 본론 첫 문장부터 시작<br>• 설화·전설이 있으면 스토리텔링 우선 서술]
        end

        SecretVault[🔐 Secret Manager<br>GEMINI_API_KEY · ADMIN_TOKEN<br>· KAKAO_MAP_KEY]
    end

    subgraph DataLayer["💾 데이터 계층 (외부 관리형 저장소)"]
        CorpusDB[("🗄️ Cloud Storage · 비공개<br><b>docent-in-hand-data</b><br>ragFullCorpus.json 5,161건<br>poi-spatial.json 2,692곳<br>서버 기동 시 1회 적재")]
        AssetDB[("📦 Cloud Storage · 비공개<br><b>docent-in-hand-assets</b><br>poi-index / poi-cards / poi 조각<br>방명록 업로드 사진")]
        StoryDB[("📒 Firestore · Native<br><b>communityStories</b><br>방명록 글 · 공감 수<br>FieldValue.increment 원자 갱신")]
    end
    ...
    %% 데이터 계층 연결
    T_Archive <-->|기동 시 적재 후 메모리 검색| CorpusDB
    T_Geo <-->|기동 시 적재 후 메모리 검색| CorpusDB
    Orchestrator <-->|"/data/* 프록시 (서비스 계정 인증)"| AssetDB
    Orchestrator <-->|"방명록 CRUD /api/stories"| StoryDB
    ...
    Orchestrator -->|3. 브리핑 노트 + 요약 요청| SummaryAgent
    SummaryAgent -->|Gemini 3.7 Flash API 호출| GeminiFlash
    GeminiFlash -->|토큰 스트림 반환| SummaryAgent
    SummaryAgent -->|4. 요약 답변 토큰 전달| Orchestrator
...
└─────────────────────────────────────────────────────────────┘
         ...
         ▼ (검증된 학술 브리핑 노트 전달)
┌─────────────────────────────────────────────────────────────┐
│ 📌 [Layer 2] 핵심 요약 에이전트 (Summary Agent)              │
│ ┌─────────────────────────────────────────────────────────┐ │
│ │ 📌 SummaryAgent : 검증된 팩트만 200~300자로 정제 전달   │ │
│ └─────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
         │
         ▼
...
### 2️⃣ Layer 2: 핵심 요약 에이전트 (`Summary Agent`)
- **목표**: 리서치 에이전트가 검증한 팩트 노트를 바탕으로 각 캐릭터 고유의 세계관에 맞추어 **100% 매끄러운 표준어 1인칭 스토리텔링** 수행.
- **단일 에이전트 (`personaAgents/summaryAgent.ts`)**: Layer 1 이 만든 브리핑 노트를 사용자용 답변으로 변환한다.
  - **감정 표현·사담 없이 검증된 학술/공간 팩트만** 전달한다.
  - **서두·종두 인사 절대 금지**: "안녕하세요", "즐거운 여행 되세요", 마크다운 소제목 없이 본론 첫 문장부터 시작한다.
  - **관광 환각 금지**: 폐사지·비지정 유적지에 "산책하기 좋습니다" 같은 임의 추천 수식어를 붙이지 않는다.
  - **설화 우선 원칙**: 대상에 설화/전설/신화 태그가 있으면 행정 연혁·면적 수치보다 스토리텔링 줄거리를 우선한다.
  - **분량**: 공백 포함 200~300자.

> 📌 **참고 — 3 페르소나에서 단일 에이전트로 통합됨**
> 초기 설계에는 설문대할망 · 해녀 삼춘 · 돌하르방 3종 페르소나 에이전트가 있었으나,
> 현재 코드는 `SummaryAgent` 하나로 통합되어 있다. `src/data/characters.ts` 의
> `CHARACTERS` 에도 `summaryAgent` 만 정의돼 있다.
> API 의 `characterId` 파라미터는 필수값 검증만 남아 있고(누락 시 400),
> 오케스트레이터는 이 값으로 분기하지 않는다.

---

## 2.5 💾 데이터 저장 계층

에이전트가 쓰는 데이터는 **컨테이너 이미지 안이 아니라 외부 관리형 저장소**에 있다.
Cloud Run 컨테이너 파일시스템은 인메모리(tmpfs)이고 인스턴스마다 격리되므로,
읽기 전용 대용량 자료와 사용자 생성 데이터를 성격에 맞게 분리했다.

| 저장소 | 대상 | 접근 경로 | 특성 |
|:---|:---|:---|:---|
| **Cloud Storage** `docent-in-hand-data` | `ragFullCorpus.json` (5,161건, gzip 5MB)<br>`poi-spatial.json` (2,692곳) | 서버가 **기동 시 1회** 다운로드 → 메모리 상주 | 읽기 전용. 브라우저에 노출되지 않음 |
| **Cloud Storage** `docent-in-hand-assets` | `poi-index.json` (62KB)<br>`poi-cards.json` (403KB)<br>`poi/{id}.json` × 2,692<br>방명록 업로드 사진 | 브라우저 → `GET /data/v{n}/*`, `GET /media/*`<br>서버가 **서비스 계정으로 중계** | 버킷은 **비공개**. 버전 프리픽스 + `immutable` 캐시 |
| **Firestore** (Native) | 컬렉션 `communityStories`<br>방명록 글 · 공감 수 | `/api/stories` CRUD | 인스턴스 간 공유. 공감은 `FieldValue.increment` 원자 갱신 |
| **Secret Manager** | `GEMINI_API_KEY`, `ADMIN_TOKEN`, `KAKAO_MAP_KEY` | 런타임 주입 (카카오 키만 빌드타임) | 저장소에 값을 남기지 않음 |

### 왜 방명록만 Firestore 인가

읽기 전용 코퍼스는 GCS 한 번 읽고 메모리에 두면 되지만, 방명록은 **여러 인스턴스가 동시에 쓰는 가변 데이터**다.
파일이나 GCS 단일 객체로 처리하면 세 가지가 깨진다.

1. **인스턴스 격리** — `max-instances=5`. 인스턴스마다 파일시스템이 달라 A 가 쓴 글을 B 가 못 본다. 파일 락은 인스턴스 로컬이라 무의미하다.
2. **휘발성** — tmpfs 이므로 인스턴스 재활용·재배포 시 소실된다.
3. **공감 카운터 경합** — read-modify-write 라 동시 요청에서 갱신이 유실된다.

Firestore 의 `FieldValue.increment` 는 서버 사이드 원자 연산이라 3번을 해결한다.
프로덕션 실측으로 **병렬 +1 요청 10회 → likes 정확히 10**, **글 작성 후 20회 분산 조회 → 20/20 일치**를 확인했다.

### 방명록 사진 처리

사진을 Firestore 문서에 base64 로 넣으면 **1MiB 문서 한도**를 넘고, `express.json` 기본 한도(100kb)에도 걸린다.

```
브라우저: 캔버스로 긴 변 1280px JPEG 축소 (보통 300KB)
  → POST /api/stories/photo  → assets 버킷 저장, /media/... 경로 반환
  → POST /api/stories        → 문서에는 URL 만 저장
  → GET  /media/*            → 서비스 계정으로 중계 (버킷은 계속 비공개)
```
...
> ℹ️ 실제 구현 엔드포인트는 `POST /api/agent/stream-story` (초기 해설) 와
> `POST /api/agent/stream-chat` (후속 Q&A) 두 개로 분리돼 있다.
> `characterId` 는 필수값 검증만 수행하며 분기에 쓰이지 않는다.
...
| 메서드 | 경로 | 설명 | 비고 |
|:---|:---|:---|:---|
| `GET` | `/api/stories?poiId={id}` | 해당 명소의 글 목록 | 공감순 → 최신순 정렬 |
| `POST` | `/api/stories` | 글 등록 | 레이트 리밋 · `data:` URL 직접 전달 시 400 |
| `POST` | `/api/stories/:id/like` | 공감 토글 | `FieldValue.increment` 원자 갱신 |
| `POST` | `/api/stories/photo` | 사진 업로드 | JPEG/PNG/WebP · 최대 4MB → `/media/...` 반환 |
| `DELETE` | `/api/admin/stories` | 전체 삭제 | `x-admin-token` 헤더 필수 (`timingSafeEqual`) |

**공통 보호 장치**: 인증 없는 공개 엔드포인트이므로 쓰기 요청에 **IP 당 분당 20회** 레이트 리밋이 걸려 있다.

### 데이터 프록시 API (Cloud Storage)

| 메서드 | 경로 | 설명 |
|:---|:---|:---|
| `GET` | `/data/v{n}/poi-index.json` | 슬림 인덱스 (62KB) — 부팅 시 1회 |
| `GET` | `/data/v{n}/poi-cards.json` | 카드 데이터 (403KB) — 탐색 시트 첫 오픈 |
| `GET` | `/data/v{n}/poi/{id}.json` | POI 상세 + RAG 문서 (~2KB) |
| `GET` | `/media/community/{file}` | 방명록 업로드 사진 |

경로는 화이트리스트 정규식으로 검증하며, 경로 탐색(`../`) 및 교차 버킷 접근은 400 을 반환한다.
GCS 의 gzip 객체를 `decompress:false` 로 통과시켜 서버에서 재압축하지 않는다.
...
│   │   └── data/
│   │       └── gcsSource.ts        # GCS 로더 (로컬 파일 폴백 지원)
│   │                                #   코퍼스는 이미지에 넣지 않고 런타임에 GCS 에서 적재
│   ├── package.json
│   └── tsconfig.json
│
...
| **답변 일관성** | 프롬프트 길이에 따라 서술 붕괴 | 검증된 팩트 노트 기반 요약에 전념 | 📌 **200~300자 정제된 팩트 요약 고정** |
